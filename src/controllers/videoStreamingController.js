...
const CONFIG = {
  MAX_TORRENTS: 2, // Keep the number of simultaneous torrents small on 4GB RAM
  TORRENT_TIMEOUT: 3 * 60 * 1000, // Drop torrents quickly once inactive
  CLEANUP_INTERVAL: 60 * 1000, // Check usage frequently to react before OOM
  FILE_CLEANUP_DELAY: 10 * 1000, // Remove files shortly after the last client leaves
  MEMORY_LIMIT_MB: 400, // Hard ceiling where we aggressively free resources
};
...
setInterval(() => {
  ...
  if (heapUsedMb > CONFIG.MEMORY_LIMIT_MB) {
    console.warn(
      `High memory usage detected (${heapUsedMb}MB). Triggering emergency cleanup.`
    );

    // Remove the oldest torrents first to release resources fast
    const sortedTorrents = Array.from(torrentLastAccess.entries()).sort(
      ([, aAccess], [, bAccess]) => aAccess - bAccess
    );

    sortedTorrents.forEach(([magnetLink]) => {
      if (
        process.memoryUsage().heapUsed / 1024 / 1024 <=
        CONFIG.MEMORY_LIMIT_MB * 0.8
      ) {
        return;
      }
...
function removeTorrent(magnetLink) {
  ...
  if (torrent) {
    ...
    try {
      ...
      if (typeof torrent.removeAllListeners === "function") {
        torrent.removeAllListeners();
      }
    } catch (err) {
      console.error(`Error preparing torrent for removal: ${err.message}`);
    }

    const removeCallback = (err) => {
      if (err) {
    ...
    if (typeof torrent.destroy === "function") {
      torrent.destroy({ destroyStore: true }, removeCallback);
    } else {
      const torrentId = torrent.infoHash || magnetLink;
      client.remove(torrentId, { destroyStore: true }, removeCallback);
    }
...
export async function streamTorrentController(req, res) {
  try {
    ...
    if (
      activeTorrents.size >= CONFIG.MAX_TORRENTS &&
      !activeTorrents.has(magnetLink)
    ) {
    ...
    req.on("close", () => {
      ...
      if (!hasOtherConnections) {
        console.log(
          `No more active connections for ${torrent.name}, scheduling removal`
        );
        setTimeout(() => {
